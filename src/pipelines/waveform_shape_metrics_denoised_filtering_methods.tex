...
A symmetric candidate edge \(\{i,j\}\) is retained when the aligned
zero-lag correlation satisfies
\[
    c_{ij}\geq\texttt{laplacian\_min\_corr}.
\]
The correlation term is first normalized to
\[
    q_{ij} =
    \frac{c_{ij}-\texttt{laplacian\_min\_corr}}
         {1-\texttt{laplacian\_min\_corr}},
    \qquad 0\leq q_{ij}\leq 1.
\]
The undirected edge weight is
\[
    w_{ij}
    =
    q_{ij}^p
...
index distance.  All threshold-passing pairs are evaluated directly, so the
weight matrix is symmetric by construction.  The Laplacian is
\[
...
\[
    (I+\gamma L)X=Y.
\]
The implementation detects connected components of the threshold graph and
solves each non-singleton component separately.  Singleton components are
returned unchanged.  Rows are then shifted back to their original phase.  By
default the solve is performed on standardized pulse shapes and restored to
each pulse's original mean and standard deviation afterward.
...
symmetric threshold graph and weighting strategy as the graph-Laplacian method,
with the \texttt{joint\_*} parameters.  The temporal graph \(L_t\) connects
...
\]
The implementation solves each non-singleton pulse-graph component spectrally.
With
\[
    A_p=I+\gamma_p L_p,
    \qquad
...
Singleton pulse-graph components are returned unchanged.  Rows are shifted
back to the original pulse phase, restored to the original pulse scale when
enabled, optionally clipped, and finally masked with the original \texttt{NaN}
locations.
...
neighbor statistics, graph degree, connected-component labels/counts, energy
values, edge counts, valid pulse count, and system condition number.
...
6 & Pulse was in a singleton graph component and was returned unchanged. \\
\bottomrule
\end{tabular}

Status codes 5 and 6 are only used by the graph-Laplacian and joint methods.  In
