...
class ArterialSegExample(ProcessPipeline):
    ...
    def _denoise_segment_block(
        ...
        out = np.asarray(v_block, dtype=float).copy()
    ...
    def _interpolate_nan_pulse(
        ...
        if not np.any(finite_mask):
            return np.full_like(pulse, np.nan)
    ...
    def _is_zero_segment(
        self,
        pulse: np.ndarray,
    ) -> bool:
        finite = pulse[np.isfinite(pulse)]

        if finite.size == 0:
            return False

        return bool(np.all(np.abs(finite) <= self.eps))

    def _remove_zero_segments(
        self,
        v_block: np.ndarray,
    ) -> tuple[np.ndarray, np.ndarray]:
        out = np.asarray(v_block, dtype=float).copy()

        _, n_beats, n_branches, n_radii = out.shape

        zero_mask = np.zeros(
            (n_beats, n_branches, n_radii),
            dtype=bool,
        )
        ...
        for beat_idx in range(n_beats):
            for branch_idx in range(n_branches):
                for radius_idx in range(n_radii):
                    ...
                    if self._is_zero_segment(pulse):
                        out[
                            :,
                            beat_idx,
                            branch_idx,
        ...
        for beat_idx in range(n_beats):
            for branch_idx in range(n_branches):
                ...
                for radius_idx in range(n_radii):
                    ...
                    if not np.all(np.isnan(pulse)):
                        valid_radii += 1

                if valid_radii < min_circles:
                    out[:, beat_idx, branch_idx, :] = np.nan
