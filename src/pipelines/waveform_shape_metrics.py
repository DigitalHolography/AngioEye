...
class WaveformShapeMetrics(ProcessPipeline):
    ...
    def _normalized_slopes_and_times(
        ...
        Q_d_skewown = float(np.nanmin(dvdt))

        return (
            float(Tbeat * s_up / (meanv + self.eps)),
            float(Tbeat * np.abs(Q_d_skewown) / (meanv + self.eps)),
            float(idx_up / v.size),
    ...
    def _metric_keys() -> list[list]:
        ...
        return [
            ...
            ["Delta_t_over_T", "((t_min-t_max) mod T)/T", "", "kinetics_and_persistence"],
    ...
    def run(self, h5file) -> ProcessResult:
        latex_formulas = {
            ...
            "Delta_t_over_T": r"$((t_{\min}-t_{\max})\bmod T)/T$",
